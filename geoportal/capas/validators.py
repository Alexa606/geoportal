...
def validate_extension(uploaded_file):
   ...
   if extension not in ALLOWED_EXTENSIONS:
       raise ValidationError(
           ...
           "Utilice Shapefile comprimido en ZIP o GeoJSON."
       )
...
def validate_shapefile_zip(uploaded_file):
   ...
   try:
       with zipfile.ZipFile(uploaded_file) as zip_file:
           ...
           for member in members:
               ...
               if path.is_absolute() or ".." in path.parts:
                   raise ValidationError(
                       "El ZIP contiene rutas no permitidas."
                   )
           ...
           if len(shp_files) > 1:
               raise ValidationError(
                   "El ZIP debe contener solamente un Shapefile."
               )
       ...
       raise ValidationError(
           "El archivo proporcionado no es un ZIP válido."
       )
...
def validate_geojson(uploaded_file):
   ...
   if not isinstance(data, dict):
       raise ValidationError(
           "El GeoJSON debe contener un objeto JSON."
       )


   if data.get("type") != "FeatureCollection":
       raise ValidationError(
           "El archivo debe ser un GeoJSON "
   ...
   if not isinstance(features, list):
       raise ValidationError(
           "El GeoJSON no contiene una lista válida de features."
       )


   if not features:
       raise ValidationError(
           "El GeoJSON no contiene entidades geográficas."
       )
